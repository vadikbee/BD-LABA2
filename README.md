...
### 1. Подготовка данных
🔮 **Очистка и работа с выбросами.** Первым шагом стала загрузка и первичный анализ данных. Ключевой особенностью предобработки является работа с сильно смещенным распределением цен. Вместо удаления выбросов, которое может привести к потере ценной информации, применен более надежный подход — логарифмическая трансформация целевой переменной (Price). Это помогает нормализовать распределение и повысить качество обучения моделей на всем диапазоне данных. Пропущенные значения в числовых признаках заполняются медианой, а в категориальных — модой.
